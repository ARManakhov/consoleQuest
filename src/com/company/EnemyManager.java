...
double speedY = speed * ((pl.getRealYPos()- realYPos) /
        ...
        if ( diametr < radius*radius & diametr > radiusOther*radiusOther){
            ...
            if (Maps.worldMap[mapNumber].borderMap[(int)realYPos/32][(int)(realXPos + speedX)/32] != 1 &&
            Maps.worldMap[mapNumber].borderMap[(int)(realYPos)/32][(int)(realXPos + ENEMY_SIZE_X + speedX)/32] != 1 &&
                    Maps.worldMap[mapNumber].borderMap[(int)(realYPos + ENEMY_SIZE_Y)/32][(int)(realXPos + speedX)/32] != 1 &&
                    Maps.worldMap[mapNumber].borderMap[(int)(realYPos + ENEMY_SIZE_Y)/32][(int)(realXPos + ENEMY_SIZE_X + speedX)/32] != 1
            ){
                realXPos = realXPos + speedX;
            }
            if(Maps.worldMap[mapNumber].borderMap[(int)(realYPos + speedY)/32][(int)realXPos/32] != 1 &&
            Maps.worldMap[mapNumber].borderMap[(int)(realYPos + speedY + ENEMY_SIZE_Y)/32][(int)(realXPos)/32] != 1 &&
                    Maps.worldMap[mapNumber].borderMap[(int)(realYPos + speedY)/32][(int)(realXPos + ENEMY_SIZE_X) /32] != 1 &&
                    Maps.worldMap[mapNumber].borderMap[(int)(realYPos + speedY + ENEMY_SIZE_Y)/32][(int)(realXPos + ENEMY_SIZE_X)/32] != 1){
